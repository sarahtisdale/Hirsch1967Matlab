...
CheckInnerAuto_Callback(hObject, eventdata, handles);
...
handles.fig7plot = plot([1 2],[1 1]); % dummy plot for now, real data in fig7plot()
handles.fig7text = text(0,0,'foo');   % dummy label for now, real label fig7plot()
guidata(hObject, handles);
...
Rc_over_Ra = str2double(handles.RcOverRa.String);
R_c = 1:-0.005:(1/Rc_over_Ra);
...
% hObject    handle to CheckInnerAuto (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
